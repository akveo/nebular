...
- add "build <package-name>" to the `build:packages` script in the `package.json`
...
2. run `npm link` for each Nebular module in _nebular/src/framework_ except for _icons_
3. make sure you don't have _node_modules_ in Nebular project (this may cause an issue an issue the angular-cli)
