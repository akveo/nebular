...
const EXPECTED_THEME_SCSS = `@forward '@beast/theme/styles/theming';
...
describe('ng-add', () => {
  ...
  describe('ng-add', () => {
    ...
    it('should add @beast/theme in package.json', function () {
      const dependencies = getPackageDependencies(tree);
      const nebularThemeVersion = require('../../package.json').version;

      expect(dependencies['@beast/theme']).toBeDefined();
      expect(dependencies['@beast/theme']).toBe(nebularThemeVersion);
    });

    it('should add @beast/eva-icons in package.json', function (done) {
      let dependencies = getPackageDependencies(tree);
      const nebularEvaIconsVersion = require('../../package.json').version;

      expect(dependencies['@beast/eva-icons']).toBeDefined();
      expect(dependencies['@beast/eva-icons']).toBe(nebularEvaIconsVersion);
  ...
  it('should register inline theme if no theme already registered', (done) => {
    runSetupSchematic({ customization: false }).subscribe(async (tree) => {
      ...
      expect(styles).toContain('./node_modules/@beast/theme/styles/prebuilt/default.css');
      done();
    });
  });
