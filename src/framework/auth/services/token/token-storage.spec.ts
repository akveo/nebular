...
describe('token-storage', () => {
  ...
  it('set test token', () => {
    const token = nbAuthCreateToken(NbAuthSimpleToken, testTokenValue, 'strategy');
  ...
  it('setter set invalid token to localStorage as empty string', () => {
    ...
    token = nbAuthCreateToken(NbAuthSimpleToken, null, 'strategy');
    tokenStorage.set(token);
    expect(localStorage.getItem(testTokenKey)).toEqual(tokenParceler.wrap(token));

    token = nbAuthCreateToken(NbAuthSimpleToken, undefined, 'strategy');
    tokenStorage.set(token);
    expect(localStorage.getItem(testTokenKey)).toEqual(tokenParceler.wrap(token));
  });
  ...
  it('should return correct value', () => {
    const token = nbAuthCreateToken(NbAuthSimpleToken, 'test', 'strategy');
