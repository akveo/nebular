...
export class NbPasswordAuthStrategy extends NbAuthStrategy {
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    ...
    return this.http.request(method, url, { body: data, observe: 'response', headers: this.options.headers })
      .pipe(
        map((res) => {
    ...
    return observableOf({})
      .pipe(
        switchMap((res: any) => {
          ...
          return this.http.request(method, url, { observe: 'response', headers: this.options.headers });
        }),
        map((res) => {
          if (this.getOption(`${module}.alwaysFail`)) {
            throw this.createFailResponse();
